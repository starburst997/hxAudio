...
class MedianFilter<T> {
    ...
    var bufferCopy:HxaArray<Float>;
    var middleIdx: Int;

    public var value(get_value, null): Float;

    public function new(size: Int = 5, defaultValue: Float) {
        buffer = new CircularBuffer<Float>(size);
        bufferCopy = HxaArrayUtil.newArray(size, true);
        middleIdx = size >> 1;
        for (i in 0...size) buffer.data[i] = defaultValue;
    }

    public function update(nextValue: Float) {
